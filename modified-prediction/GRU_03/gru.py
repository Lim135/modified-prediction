...
test_set = pd.concat([maotai.iloc[1529 - 300:, 3:4],
                      ...
                      yutongkeji.iloc[1529 - 300:, 3:4]], axis=1).values # 后300天的开盘价作为测试集
...
model = tf.keras.Sequential([
    GRU(80, return_sequences=True),  # 定义了一个具有80个记忆体的GRU层，这一层会在每个时间步输出ht
    Dropout(0.2),
    GRU(100), # 定义了一个具有100个记忆体的GRU层，这一层仅在最后一个时间步输出ht
    Dropout(0.2),
    Dense(1)
])
# 配置训练方法
...
checkpoint_save_path = "./checkpoint_gru/"
...
checkpoint_save_path  += "/gru_stock.ckpt"
...
# 参数提取
file = open('./gru_stock_weights.txt', 'w')
